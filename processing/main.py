...
codes = {
    ...
    OUTLINES_WHITE  : load('outlines_white'),
    COMIC           : load('comic'),
    SIMPLE_CARTOON  : load('simple_cartoon')
}
...
class Texture():
    ...
    def __init__(self, source):
        if type(source) == moderngl.Texture:
            ...
            self.tex = ctx.texture((source.shape[1], source.shape[0]), components=source.shape[2], data=source.copy(), dtype="f4")
    ...
    def toSurface(self) -> pygame.Surface:
        ...
        surf = pygame.image.frombytes(buffer, self.tex.size, 'RGBA', True)
        return surf
